...
export function MemberShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    void memberApi<{ branding?: { displayName: string; primaryColor: string; secondaryColor: string }; gymSelectionRequired?: boolean; gymOptions?: Array<{ gymRef: string; displayName: string }> }>("/api/v1/member/me")
      .then((data) => {
  ...
  async function logout() {
    try { await memberApi("/api/v1/auth/logout", { method: "POST" }); } finally { window.sessionStorage.removeItem("afc_member_csrf"); window.sessionStorage.removeItem("afc_member_gym"); window.location.assign("/app/login"); }
  }
  return <div className="member-shell" dir="rtl" style={{ "--member-primary": branding.primaryColor, "--member-secondary": branding.secondaryColor } as React.CSSProperties}>
    <header className="member-topbar">
      <Link href="/app" className="member-brand"><span>AF</span><strong>{branding.displayName}</strong></Link>
    ...
    <main className="member-content">{gymSelection.required ? <section className="member-card member-gym-picker"><p className="member-kicker">اختيار القاعة</p><h2>حدد القاعة اللي تريد تفتح بياناتها</h2><select defaultValue="" onChange={(event) => { if (event.target.value) { window.sessionStorage.setItem("afc_member_gym", event.target.value); window.location.reload(); } }}><option value="" disabled>اختر القاعة</option>{gymSelection.options.map((gym) => <option value={gym.gymRef} key={gym.gymRef}>{gym.displayName}</option>)}</select></section> : children}</main>
    <nav className="member-bottom-nav" aria-label="تنقل العضو">{links.map((link) => <Link className={pathname === link.href ? "active" : ""} href={link.href} key={link.href}><span>{link.icon}</span>{link.label}</Link>)}</nav>
  </div>;
}
